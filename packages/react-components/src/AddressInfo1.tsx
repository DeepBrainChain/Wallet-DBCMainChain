...
import { BlockToTime, FormatBalance, FormatBalanceNew } from '@polkadot/react-query';
...
function createBalanceItems (formatIndex: number, lookup: Record<string, string>, t: TFunction, 
  { address, balanceDisplay, balancesAll, bestNumber, convictionLocks, democracyLocks, isAllLocked, otherBonded, ownBonded, stakingInfo, votingOf, withBalanceToggle, withLabel }: 
  { address: string; balanceDisplay: BalanceActiveType; balancesAll?: DeriveBalancesAll | DeriveBalancesAccountData; bestNumber?: BlockNumber; convictionLocks?: RefLock[]; democracyLocks?: DeriveDemocracyLock[]; isAllLocked: boolean; otherBonded: BN[]; ownBonded: BN; stakingInfo?: DeriveStakingAccount; votingOf?: Voting; withBalanceToggle: boolean, withLabel: boolean }): React.ReactNode {
...
function createBalanceItems1 (formatIndex: number, lookup: Record<string, string>, t: TFunction, 
  ...
  //   </React.Fragment>
  // );
  dlcBalances && !dlcBalances.balance.isZero() && allItems.push(
    <React.Fragment key={`dlcBalances:${1}`}>
      ...
      <React.Fragment key={formatIndex}>
        <Expander
          summary={
            <FormatBalanceNew
              formatIndex={formatIndex}
              value={dlcBalances_Add&&dlcBalances_Add.balance.add(dlcBalancesLocks_Add)}
            />
          }
        >
...
function renderBalances1 (props: Props, lookup: Record<string, string>, bestNumber: BlockNumber | undefined, t: TFunction): React.ReactNode[] {
  const { address, balancesAll, convictionLocks, democracyLocks, stakingInfo, dlcBalances = DlcBalances_Defult, dlcBalancesLocks = DlcBalancesLocks_Defult, dlcBalances_Add = DlcBalances_Defult, dlcBalancesLocks_Add = DlcBalancesLocks_Defult, votingOf, withBalance = true, withBalanceToggle = false, withLabel = false } = props;
  const balanceDisplay = withBalance === true
    ? DEFAULT_BALANCES
    : withBalance || false;
...
function AddressInfo (props: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={`${className} ui--AddressInfo ${withBalanceToggle ? 'ui--AddressInfo-expander' : ''}`}>
      <div className={`column${withBalanceToggle ? ' column--expander' : ''} eeeee`}>
      ...
      <div className={`column${withBalanceToggle ? ' column--expander' : ''} sssss`}>
        {renderBalances1(props, lookup.current, bestNumber, t)}
        {withHexSessionId && withHexSessionId[0] && (
          <>
            <Label label={t<string>('session keys')} />
            <div className='result'>{withHexSessionId[0]}</div>
