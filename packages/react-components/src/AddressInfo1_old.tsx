...
import type { BlockNumber, ValidatorPrefsTo145, Balance, Voting } from '@polkadot/types/interfaces';
...
function createBalanceItems1 (formatIndex: number, lookup: Record<string, string>, t: TFunction, 
  ...
  dlcBalances && !(dlcBalances.isEmpty) && allItems.push(
    <React.Fragment key={`dlcBalances:${1}`}>
      <Label label={dlcBalances.value.isFrozen == true ? t<string>("DLC_frozen") : t<string>("transferrable")} />
      <FormatBalanceNew
        className="result"
        formatIndex={formatIndex}
        labelPost={<IconVoid />}
      ...
      <React.Fragment key={formatIndex}>
        <Expander
          summary={
            <FormatBalanceNew
              formatIndex={formatIndex}
              value={dlcBalances_Add&&dlcBalances_Add.balance}
            />
          }
        >
